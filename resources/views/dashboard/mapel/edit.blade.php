...
@section('container')
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
      ...
      <div class="col-lg-8">
          <form method="POST" action="/dashboard/mapel/ {{ $mapel->id }}">
            @csrf
            @method('PUT')
            <div class="col-lg">
              <label for="mapel" class="form-label">Nama Mata Pelajaran</label>
              <input type="text" class="form-control @error('mapel') is-invalid @enderror" id="mapel" name="mapel" required value="{{ old('mapel', $mapel->mapel) }}">
              @error('mapel')
            ...
            <div class="col-lg mt-2">
              ...
              <select name="guru_id" id="cari" data-placeholder="Pilih Wali Kelas" class="form-select @error('guru_id') is-invalid @enderror" required>
                ...
                @if (old('guru_id', $mapel->guru_id === $item->id))
